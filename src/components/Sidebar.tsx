    ...
    SidebarProps) => {
        ...
        const handleRollback = async (versionId: string) => {
           try {
            ...
            const { data: data2} = await api.get(`/api/project/rollback/${project.id}`)
        ...
        const handleRevisions = async (e: FormEvent) => {
            ...
            setProject((currentProject) => currentProject ? {
                ...currentProject,
                conversation: [...currentProject.conversation, userMessage],
            } : currentProject);
            ...
            try {
                await api.post(`/api/project/revision/${project.id}`, { message: prompt });
                const pollInterval = window.setInterval(async () => {
                    try {
                        ...
                        const latestAssistantMessage = [...updatedProject.conversation]
                            .reverse()
                ...
                toast.error(getApiErrorMessage(error, "Failed to apply revision"));
                setProject((currentProject) => currentProject ? {
                    ...currentProject,
                    conversation: currentProject.conversation.filter((message) => message.id !== userMessage.id),
                } : currentProject);
                setIsGenerating(false)
            }
        }
