...
import type { PreviewElement, PreviewElementUpdate, Project } from "../types";
...
(({project, isGenerating, aiStatus, device = 'desktop', showEditorPanel = true}, ref) => {
    ...
    const handleUpdate = (updates: PreviewElementUpdate)=> {
    ...
    return (
        <div className="relative h-full bg-gray-900 flex-1 rounded-xl overflow-hidden max-sm:ml-2">
            {project.current_code ? (
                ...
                className={`h-full max-sm:w-full ${resolutions[device]} max-auto transition-all`}
                />
                ...
                <LoaderSteps statusLabel={aiStatus} />
            )}
        </div>
    )
})
