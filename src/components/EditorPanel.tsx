...
import type { PreviewElement, PreviewElementStyles, PreviewElementUpdate } from "../types";


interface EditorPanelProps {
    selectedElement: PreviewElement | null;
    onUpdate: (updates: PreviewElementUpdate)=> void;
    onClose: ()=> void;
}
...
const EditorPanel = ({selectedElement, onUpdate, onClose}: EditorPanelProps) => {
    ...
    const handleChange = (field: "text" | "className", value: string)=> {
        ...
        setValues(newValues)
        onUpdate(field === "text" ? { text: value } : { className: value });
    }

    const handleStyleChange = (styleName: keyof PreviewElementStyles, value: string) => {
        const newStyles = {...values.styles, [styleName]: value};
        setValues({...values, styles: newStyles});
        onUpdate({styles: {[styleName]: value} as Partial<PreviewElementStyles>})
    }
