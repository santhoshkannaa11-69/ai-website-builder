    ...
    return (
        ...
        <Link to='/'>
          ...
          <div className="hidden md:flex items-center gap-8 transition duration-500">
            ...
            { !session?.user ? (
              <button onClick={()=> navigate('/auth/sign-in')} className="px-6 py-1.5 max-sm:text-sm bg-indigo-600 active:scale-95 hover:bg-indigo-700 transition rounded">
              Get started
            </button>
            ) : (
              <>
              <button className="bg-white/10 px-5 py-1.5 text-xs sm:text-sm border text-gray-200 rounded-full">
              Credits: <span className="text-indigo-300">{isCreditsLoading ? "..." : credits}</span>
              </button>
            ...
            <button aria-label="Open mobile menu" id="open-menu" className="md:hidden active:scale-90 transition" onClick={() => setMenuOpen(true)} >
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-[100] bg-black/60 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300">
            ...
            <button aria-label="Close mobile menu" className="active:ring-3 active:ring-white aspect-square size-10 p-1 items-center justify-center bg-slate-100 hover:bg-slate-200 transition text-black rounded-md flex" onClick={() => setMenuOpen(false)} >
