...
const LoaderSteps = ({ statusLabel }: LoaderStepsProps) => {
    ...
    const resolvedStep = normalizedStatus.includes('step 1')
        ? 0
        : normalizedStatus.includes('step 2')
        ? 1
        : normalizedStatus.includes('step 3')
        ? 2
        : normalizedStatus.includes('step 4')
        ? 3
        : current;

    const Icon = steps[resolvedStep].icon
    const label = statusLabel || steps[resolvedStep].label
    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-center bg-gray-950 relative overflow-hidden text-white">
            ...
            <p className="mt-8 text-lg font-light text-white/90 tracking-wide transition-all duration-700 ease-in-out opacity-100" key={label}>{label}</p>
